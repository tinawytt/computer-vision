...
class MultipleOutputLoss(nn.Module):
    def __init__(self, weight_factors=None):
        ...
        self.loss = DC_and_CE_loss({'batch_dice': True, 'smooth': 1e-5, 'smooth_in_nom': True,'do_bg': False, 'rebalance_weights': None, 'background_weight': 1}, OrderedDict())
    ...
    def forward(self, x, y):
        ...
        losslist=[]
        print(len(x))
        loss_layer1=self.loss(x[0],y)
        loss_layer2=self.loss(x[1],y)
        loss_layer3=self.loss(x[2],y)
        loss_layer4=self.loss(x[3],y)
        print("move here")
        # losslist.append(loss_layer1)
        losslist.append(loss_layer2)
        losslist.append(loss_layer3)
        losslist.append(loss_layer4)
        idx=losslist.index(min(losslist))
        idx=idx+1
        weights[idx]=2
        l =(weights[0] * loss_layer1+weights[1] * losslist[0]+weights[2] * losslist[1]+weights[3] * losslist[2]) /7
        # for i in range(1, len(x)):
        #    if weights[i] != 0:
        #        l += weights[i] * losslist[i]
...
class UNetExperiment3D(PytorchExperiment):    
    def setup(self):
        ...
        self.model = UNet3DPlus(num_classes=self.config.num_classes, in_channels=self.config.in_channels)
        self.model = torch.nn.DataParallel(self.model) # 多卡并行
        self.model.to(self.device)
        
        self.loss =MultipleOutputLoss() 
    ...
    def validate(self, epoch):
        ...
        with torch.no_grad():
            ...
            for data_batch in self.val_data_loader:
                ...
                #     self.add_result(value=loss.item(), name='val_Loss', tag='Loss', counter=epoch + (batch_counter / self.val_data_loader.data_loader.num_batches))
